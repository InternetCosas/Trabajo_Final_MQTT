...
const uint8_t localAddress = 0x81;     // Dirección de este dispositivo
uint8_t destination = 0x80;            // Dirección de destino, 0xFF es la dirección de broadcast
...
LoRaConfig_t thisNodeConf   = { 6, 12, 5, 2};   // Configuración del esclavo
int remoteRSSI = 0;
float remoteSNR = 0;
int packetCount = 0;
bool ack = false;
...
void onReceive(int packetSize) 
{
  ...
  // Actualizamos la configuración del escalvo y la mostramos
  if (receivedBytes == 4) {
    thisNodeConf.bandwidth_index = buffer[0] >> 4;
    thisNodeConf.spreadingFactor = 6 + ((buffer[0] & 0x0F) >> 1);
    thisNodeConf.codingRate = 5 + (buffer[1] >> 6);
    thisNodeConf.txPower = 2 + ((buffer[1] & 0x3F) >> 1);
    remoteRSSI = -int(buffer[2]) / 2.0f;
    remoteSNR  =  int(buffer[3]) - 148;
    Serial.print("New config: BW: ");
    ...
    Serial.println(" dB\n");
    SerialUSB.println("Changing configuration...");
    ...
    SerialUSB.println("Configuration changed\n");
    ack = true;
  }
  else {

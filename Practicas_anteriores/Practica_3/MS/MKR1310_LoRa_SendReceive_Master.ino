...
const uint8_t localAddress = 0x90;     // Dirección de este dispositivo
uint8_t destination = 0x91;            // Dirección de destino, 0xFF es la dirección de broadcast
...
// Variables de monitorización de estado de conexión.
bool blockedValue[] = {false, false, true, false};
int actualAvgRSSI = -127;
int lastAvgRSSI = -127;
float actualAvgSNR = -148;
float lastAvgSNR = -148;
int lastTx = 10000;
int currentTx = 9999;
int packagesCount = 0;
// -------------------------------------------------
...
// 0: se calibran los valores
// 1: se manda un paquete
// 2: se actualizan los parámetros del Maestro

  currentMillis = millis();
  if( !transmitting && (currentMillis - startMillis >= txInterval_ms)){
    if(status == 0)
      calibrateValues();
...
// ------------------------------------------------------------------
// Actualizamos el estado, cambiamos parámetros
// y actualizamos los últimos valores de RSSI y SNR
    packagesCount = 0;
    status = newStatus;
    ...
    actualAvgRSSI = lastAvgRSSI;
    actualAvgSNR = lastAvgSNR;
...
// ------------------------------------------------------------------
  ...
  lastAvgRSSI = (LoRa.packetRssi() + remoteRSSI)/2;
  lastAvgSNR = (LoRa.packetSnr() + remoteSNR)/2;
}
...
// Se calibran los valores para maximizar RSSI y SNR
void calibrateValues(){
  Serial.println("Calibrando valores...");
  status = 1;
  int currentField = -1;

  for(int i=0; i<4; i++){
    if(blockedValue[i]==false){
      currentField = i;
      break;
    }
  }
  ...
  {
    case 0:
      if(thisNodeConf.codingRate > 5){
        thisNodeConf.codingRate -= 1;
        Serial.println("Se decrementa codingRate...");
      }
      else{
        Serial.println("codingRate ya ha llegado a su límite...");
        status = 0;
        blockedValue[currentField] = true;
      }
      break;
    case 1:
      if(thisNodeConf.spreadingFactor > 7){
        thisNodeConf.spreadingFactor -= 1;
        Serial.println("Se decrementa spreadingFactor...");
      }
      else{
        Serial.println("spreadingFactor ya ha llegado a su límite...");
        status = 0;
        blockedValue[currentField] = true;
      }
      break;
    case 2:
      if(thisNodeConf.txPower < 20){
        thisNodeConf.txPower += 1;
        Serial.println("Se incrementa txPower...");
      }
      else{
        Serial.println("txPower ya ha llegado a su límite...");
        status = 0;
        blockedValue[currentField] = true;
      }
      break;
    case 3:
      if(thisNodeConf.bandwidth_index < 9){
        ...
        Serial.println("Se incrementa bandwidth_index...");
      }
      else{
        Serial.println("bandwidth_index ya ha llegado a su límite...");
        status = 0;
        blockedValue[currentField] = true;
      }
      break;
    default:
      status = 0;
      blockedValue[0] = false;
      blockedValue[1] = false;
      blockedValue[2] = true;
      blockedValue[3] = false;
      Serial.println("Ya se han ajustado todos los valores, se vuelve a comenzar el calibrado...");
    break;
  }

  if((lastAvgRSSI < actualAvgRSSI || lastAvgSNR < actualAvgSNR) && lastTx > currentTx )
  {
    switch(currentField)
    {
      case 0:
        ...
        lastAvgRSSI = -127;
        lastAvgSNR = -148;
        Serial.println("Los valores anteriores eran más óptimos. Se establece anterior codingRate...");
        break;
      case 1:
        thisNodeConf.spreadingFactor += 1;
        blockedValue[currentField] = true;
        lastAvgRSSI = -127;
        lastAvgSNR = -148;
        Serial.println("Los valores anteriores eran más óptimos. Se establece anterior spreadingFactor...");
        break;
      case 2:
        thisNodeConf.txPower -= 1;
        blockedValue[currentField] = true;
        lastAvgRSSI = -127;
        lastAvgSNR = -148;
        Serial.println("Los valores anteriores eran más óptimos. Se establece anterior txPower...");
        break;
      case 3:
        thisNodeConf.bandwidth_index -= 1;
        blockedValue[currentField] = true;
        lastAvgRSSI = -127;
        lastAvgSNR = -148;
        Serial.println("Los valores anteriores eran más óptimos. Se establece anterior bandwidth_index...");
        break;
    }
  }
}

// Se aplican los cambios de parámetro del Maestro
void changeParams(){
  Serial.println("Master está aplicando una nueva configuración...");
  LoRa.setSignalBandwidth(long(bandwidth_kHz[thisNodeConf.bandwidth_index])); 
  LoRa.setSpreadingFactor(thisNodeConf.spreadingFactor);     
  LoRa.setCodingRate4(thisNodeConf.codingRate);                  
  LoRa.setTxPower(thisNodeConf.txPower, PA_OUTPUT_PA_BOOST_PIN); 
  Serial.println("Master ha aplicado una nueva configuración...");
  Serial.print("Master config: BW: ");
  Serial.print(String(bandwidth_kHz[thisNodeConf.bandwidth_index]));
  Serial.print(" kHz, SPF: ");
  Serial.print(String(thisNodeConf.spreadingFactor));

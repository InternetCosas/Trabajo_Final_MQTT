...
*/
    ...
    for (int tid = 1; tid < NUM_THREADS; tid++) {
      ...
      if (tid == 4) {         // Controlamos el hilo idle
          if (thdLoad->loadPerCycle_per <= 15) { // Si su carga es menor o igual al 15% decrementamos la carga de los hijos con valores aleatorios entre 25 y 100
             threadLoad[1] -= random(25, 100);
             threadLoad[2] -= random(25, 100);
             threadLoad[3] -= random(25, 100);
          } else {                              // En caso que sea mayor al 15% aumentamos la carga de sus hijos con valores aleatorios entre 15 y 50
             threadLoad[1] += random(15, 50);
             threadLoad[2] += random(15, 50);
             threadLoad[3] += random(15, 50);
          }
      }
    }
    SerialUSB.println();

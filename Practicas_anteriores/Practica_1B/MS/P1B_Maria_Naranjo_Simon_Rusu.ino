...
int incremento = 1; // Incremento inicial para aproximación exponencial.
int umbral = 15;    // Umbral de CPU Ociosa.
...
//--------------------------------------------------------------------------------------
// PRACTICA 1B -SIMON RUSU y MARIA NARANJO-
//--------------------------------------------------------------------------------------

      if (tid == 4){ // Hebra idle 
          if (thdLoad->loadPerCycle_per > umbral){
            
            SerialUSB.println("Mas del 15% de CPU ociosa, incremento de carga en las hebras.");

            incremento *=2; //aproximación exponencial por la parte superior del umbral.
            threadLoad[1] += incremento;
            threadLoad[2] += incremento;
            threadLoad[3] += incremento;
          }
          else if (thdLoad->loadPerCycle_per < umbral){
            
            SerialUSB.println("Mas del 85% de CPU en uso, decremento de carga en las hebras.");

            if (umbral - thdLoad->loadPerCycle_per <= 0.5) //Aproximación a un 15% de CPU Ociosa. Provoca su oscilación entre 14% y 15% de carga.
            {
              incremento = 1;
            }
            else incremento /=2; //aproximación exponencial por la parte inferior del umbral.
...
//--------------------------------------------------------------------------------------
      ...
      int randomIncrease = 1 + (rand() % 10); // Frecuencia aleatoria de aumento de carga.
      int randomThread = 1 + (rand() % 3); // Elige una hebra para aumentar su carga de forma aleatoria.
      
      if(thdLoad->loadPerCycle_per < umbral && thdLoad->loadPerCycle_per > umbral-1 && randomIncrease == 1 ){
         int randomValue = 1 + (rand() % 100); //Aumento de carga de 1 a 100.
         SerialUSB.print("El hilo ");
         SerialUSB.print(randomThread);
         SerialUSB.print(" ha incrementado un ");
         SerialUSB.print(randomValue);
         SerialUSB.println("% su carga");
        
         threadLoad[randomThread] += randomValue;
         incremento = randomValue;
      }
    }
